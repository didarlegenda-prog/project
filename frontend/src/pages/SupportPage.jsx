...
import { HelpCircle, Plus, X, ChevronUp } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={() => setShowNewTicketForm(!showNewTicketForm)}>
            {showNewTicketForm ? <X className="h-4 w-4 mr-2" /> : <Plus className="h-4 w-4 mr-2" />}
        ...
        {showNewTicketForm && (
          <div className="mb-8 bg-white rounded-lg shadow-md p-6 border-2 border-blue-500 animate-slide-down">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-dark-900">Create Support Ticket</h2>
              <button onClick={() => { setShowNewTicketForm(false); reset(); }} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" aria-label="Close form">
            ...
            <form onSubmit={handleSubmit(handleCreateTicket)} className="space-y-4">
              ...
              <div className="flex space-x-3">
                ...
                <Button
                  ...
                  variant="outline"
                  fullWidth
                  onClick={() => { setShowNewTicketForm(false); reset(); }}
                >
                  Cancel
                </Button>
              </div>
        ...
        {tickets.length === 0 ? (
          <EmptyState
            ...
            action={
              <Button variant="warning" size="lg" fullWidth onClick={() => setShowNewTicketForm(true)}>
              ...
              <div key={ticket.id} className="space-y-4">
                ...
                {expandedTicketId === ticket.id && selectedTicket && (
                  <div className="bg-white rounded-lg shadow-md p-6 border-2 border-blue-500 animate-slide-down">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-dark-900 text-lg">Ticket #{selectedTicket.id}</h3>
                      <button onClick={() => { setExpandedTicketId(null); setSelectedTicket(null); }} className="p-1 hover:bg-gray-100 rounded" aria-label="Collapse ticket details">
                        <ChevronUp className="h-5 w-5 text-gray-500" />
                      </button>
                    </div>
                    <div>
                      <h4 className="font-semibold text-dark-900 mb-2">{selectedTicket.subject}</h4>
                    ...
                    {selectedTicket.comments && selectedTicket.comments.length > 0 && (
                      <div className="border-t border-dark-200 pt-4 mt-4">
                        <h4 className="font-semibold text-dark-900 mb-3">Comments</h4>
